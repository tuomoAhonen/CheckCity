let pyynnot = [
    { nimi: "Ravintola Kanokchai", kuvaus: "Thaimaalainen ravintola", kotisivu: "", leveysaste: 60.17505389362412, pituusaste: 24.928911368008897, sijainninkuvaus: "Cygnaeuksenkadun ja Museokadun kulmasta noin 10 metriä, kadun oikealla puolella.", katuosoite: "Cygnaeuksenkatu 5", postinumero: "00100", kaupunki: "Helsinki", maa: "Suomi", kategoria: "Ravintolat", display: 0, aktiivinen: 0 },
    { nimi: "EB-hair", kuvaus: "Parturi-kampaamo", kotisivu: "https://www.ebhair.fi", leveysaste: 60.184305288933366, pituusaste: 24.923810291923964, sijainninkuvaus: "Katsottuna mannerheimintieltä, se sijaitsee kauneus salongin oikealla puolella, viereinen liike. Hesburgerista vasemmalle, noin 10 metriä.", katuosoite: "Mannerheimintie 19", postinumero: "00250", kaupunki: "Helsinki", maa: "Suomi", kategoria: "Palvelut", display: 0, aktiivinen: 0 },
];
let url = "";
let muokattavaindex = 0;

if (document.getElementById("pyynnot")) {
    console.log("Vain pyynnot osuus latautuu.");
    function pyynnotDisplay() {
        let teksti = "";

        teksti = "<div class='pyynnotdiv'>";
        
        for (let i = 0; i < pyynnot.length; i++) {
            teksti = teksti+"<div class='pyynto'>";
            teksti = teksti+"<div class='pyyntodivs' id='numero'>"+(i+1)+"</div>";
            teksti = teksti+"<div class='pyyntodivs' id='nimi'>"+pyynnot[i].nimi+"</div>";
            teksti = teksti+"<div class='pyyntodivs' id='kuvaus'>"+pyynnot[i].kuvaus+"</div>";
            teksti = teksti+"<div class='pyyntodivs' id='kategoria'>"+pyynnot[i].kategoria+"</div>";
            teksti = teksti+"<div class='pyyntodivs' id='muokkaa'><button type='button' onclick='muokkaaPyyntoa("+i+")' id='' class='buttons'>Muokkaa</button></div>";
            teksti = teksti+"<div class='pyyntodivs' id='poista'><button type='button' onclick='poistaPyynto("+i+")' id='' class='buttons'>Poista</button></div>";
            teksti = teksti+"</div>";
        }
        
        teksti = teksti+"</div>";

        document.getElementById("pyynnot").innerHTML = teksti;
    };

    pyynnotDisplay();

    function muokkaaPyyntoa(index) {
        //sessionStorage.setItem(0, index);
        return location.href = "lisaakohde.html?"+index;
    };

    function poistaPyynto(index) {
        //väliaikainen ratkaisu, pitää tehdä api muotoon
        pyynnot.splice(index, 1);
        //console.log(pyynnot);
        return pyynnotDisplay();
    };
};

let map;

if (document.getElementById("kartta")) {
    console.log("Vain map osuus latautuu.")
    //console.log(sessionStorage.getItem(0)+" session storage value");
    //sessionStorage.clear();
    url = location.href;
    if(parseInt(url.slice(url.indexOf("?")+1)) || parseInt(url.slice(url.indexOf("?")+1)) == 0) {
        muokattavaindex = url.slice(url.indexOf("?")+1);

        document.getElementById("nimi").value = pyynnot[muokattavaindex].nimi;
        document.getElementById("kuvaus").value = pyynnot[muokattavaindex].kuvaus;
        document.getElementById("kotisivu").value = pyynnot[muokattavaindex].kotisivu;
        document.getElementById("kategoria").value = pyynnot[muokattavaindex].kategoria;
        document.getElementById("katuosoite").value = pyynnot[muokattavaindex].katuosoite;
        document.getElementById("postinumero").value = pyynnot[muokattavaindex].postinumero;
        document.getElementById("leveysaste").value = pyynnot[muokattavaindex].leveysaste;
        document.getElementById("pituusaste").value = pyynnot[muokattavaindex].pituusaste;
        document.getElementById("sijainninkuvaus").value = pyynnot[muokattavaindex].sijainninkuvaus;  
    }
    ...
    function initMap() {
        ...
        if(parseInt(url.slice(url.indexOf("?")+1)) || parseInt(url.slice(url.indexOf("?")+1)) == 0) {
            ...
            let marker = new google.maps.Marker({
                position: new google.maps.LatLng(pyynnot[muokattavaindex].leveysaste, pyynnot[muokattavaindex].pituusaste),
                icon: "pyyntomarkerfinal.png",
                map: map,
            });

            let infoWindow = new google.maps.InfoWindow({
                content: "<p><b>"+pyynnot[muokattavaindex].nimi+"</b><br/>"+pyynnot[muokattavaindex].kuvaus+"<br/>"+pyynnot[muokattavaindex].katuosoite+", "+pyynnot[muokattavaindex].postinumero+" "+pyynnot[muokattavaindex].kaupunki+"</p>",
            });
